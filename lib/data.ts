...
// Commission math lives in lib/commission.ts (extracted during Phase 7
// structure polish — self-contained, safety-critical, fully tested).
...
export function computeIncentiveProgress(
  ...
  switch (incentive.metric) {
    ...
    case 'commission': {
      ...
      const closerRepsByProject = new Map(
        relevantProjects.map((p) => [
          p.id,
          new Set([p.repId, ...(p.additionalClosers?.map((c) => c.userId) ?? [])]),
        ])
      );
      return payrollEntries
        .filter((e) =>
          ...
          (closerRepsByProject.get(e.projectId)?.has(e.repId) ?? false) &&
          e.notes !== 'Setter'
        )
        .reduce((s, e) => s + e.amount, 0);
    }
