...
export interface Project {
  ...
  m3Amount?: number;
  m3Paid: boolean;
  // Setter-specific M1/M2/M3 amounts (closer's values live in m1Amount/m2Amount/m3Amount)
...
// ── Commission math using exact-cent Money internally ──────────────────────
//
// Public signatures still return `number` (dollars) so every call site keeps
// working. Inside we use `Money` (integer cents) so multi-step arithmetic
// (halves, percent allocations, sums) doesn't accumulate float drift. See
// lib/money.ts for the contract. The ONLY float↔cent rounding happens at
// fromDollars() boundaries — every downstream op is integer-exact.

import * as $ from './money';

/** Core formula in Money terms: max(0, (soldPPW - baseline) × kW × 1000). */
function commissionMoney(soldPPW: number, baselinePerW: number, kW: number): $.Money {
