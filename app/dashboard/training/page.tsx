...
function TrainingPageInner() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl animate-fade-in-up">
      ...
      {/* ── Tab bar ── */}
      <div className="flex flex-wrap gap-1 mb-6 bg-slate-900 border border-slate-800 rounded-xl p-1 w-fit tab-bar-container">
        ...
        {TABS.map((t, i) => (
          <button
            ...
            className={`relative z-10 px-4 py-2 rounded-lg text-sm font-medium transition-colors active:scale-[0.97] ${
              activeTab === t.key ? 'text-white' : 'text-slate-400 hover:text-white'
            }`}
          >
            {t.label}
          </button>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-slate-500 text-[10px] font-semibold uppercase tracking-widest">Total Earned</span>
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-slate-500 text-[10px] font-semibold uppercase tracking-widest">Active Trainees</span>
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-slate-500 text-[10px] font-semibold uppercase tracking-widest">Avg Override Rate</span>
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <span className="text-slate-500 text-[10px] font-semibold uppercase tracking-widest">Trainee Deals</span>
                <BarChart2 className="w-4 h-4 text-emerald-400/50" />
              </div>
              <p
                className="text-3xl font-black tabular-nums text-emerald-400 stat-value"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="flex items-center gap-3 bg-slate-800/40 rounded-xl px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-emerald-400 font-bold tabular-nums">{fmt$(totalEarned)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3 bg-slate-800/40 rounded-xl px-4 py-3">
              ...
              <div className="flex items-center gap-3 bg-slate-800/40 rounded-xl px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-slate-400 font-bold tabular-nums">{fmt$(draftAmount)}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-slate-800 text-white rounded-xl pl-10 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50 placeholder-slate-500"
              />
            </div>
            {traineeSearch && (
              <span className="text-xs text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full">{filteredTrainees.length} result{filteredTrainees.length !== 1 ? 's' : ''}</span>
            )}
              ...
              {(['name', 'deals', 'earnings'] as const).map((s) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                    traineeSort === s ? 'bg-amber-500/15 text-amber-400' : 'text-slate-400 hover:text-white'
                  }`}
                >
          ...
          {filteredTrainees.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3 rounded-xl bg-slate-900/30 border border-dashed border-slate-800">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">{traineeSearch ? 'Try adjusting your search query or sort criteria' : 'Trainer assignments will populate trainees automatically'}</p>
              </div>
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <Link href={`/dashboard/reps/${t.traineeId}`} className="text-white font-bold text-sm truncate hover:text-blue-300 transition-colors">{t.traineeName}</Link>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <p className="text-slate-500 text-[10px]">earned</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {!t.nextThreshold && (
                        <span className="text-slate-500 text-[10px]">Final tier</span>
                      )}
                    </div>
      ...
      {activeTab === 'payments' && (
        <div key="payments" className="animate-tab-enter space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            {paymentSearch && (
              <span className="text-xs text-slate-500 bg-slate-800 px-2 py-0.5 rounded-full">{filteredPayments.length} result{filteredPayments.length !== 1 ? 's' : ''}</span>
            )}
            <div className="flex gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1 w-fit">
              {(['all', 'Draft', 'Pending', 'Paid'] as const).map((s) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                    paymentStatusFilter === s ? 'bg-amber-500/15 text-amber-400' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  {s === 'all' ? 'All' : s}
                </button>
          ...
          {filteredPayments.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 gap-3 rounded-xl bg-slate-900/30 border border-dashed border-slate-800">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-slate-500 mt-1">{paymentSearch || paymentStatusFilter !== 'all' ? 'Try adjusting your search or status filter' : 'Override payments appear here when trainees close deals'}</p>
              </div>
            </div>
          ) : (
            <div className="card-surface rounded-2xl overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="table-header-frost border-b border-slate-800">
                    <tr>
                      <th className="text-left px-4 py-3 text-slate-400 text-xs font-semibold uppercase tracking-wider">Customer</th>
                      <th className="text-left px-4 py-3 text-slate-400 text-xs font-semibold uppercase tracking-wider">Trainee</th>
                      <th className="text-right px-4 py-3 text-slate-400 text-xs font-semibold uppercase tracking-wider">Amount</th>
                  ...
                  <tbody className="divide-y divide-slate-800/50">
                    {filteredPayments.map((entry) => (
                      <tr key={entry.id} className="hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-slate-300 text-sm">{(() => { const t = getTraineeForEntry(entry); return t ? <Link href={`/dashboard/reps/${t.id}`} className="hover:text-blue-300 transition-colors">{t.name}</Link> : '—'; })()}</td>
                        ...
                        <td className="px-4 py-3 text-slate-400 text-sm">{new Date(entry.date + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {traineeData.map((t, idx) => (
            ...
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <Link href={`/dashboard/reps/${t.traineeId}`} className="text-white font-bold text-sm hover:text-blue-300 transition-colors">{t.traineeName}</Link>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead className="table-header-frost">
                    <tr className="border-b border-slate-800">
                      <th className="text-left px-3 py-2 text-slate-400 text-xs font-semibold uppercase tracking-wider">Tier</th>
                      <th className="text-left px-3 py-2 text-slate-400 text-xs font-semibold uppercase tracking-wider">Deal Range</th>
                      <th className="text-right px-3 py-2 text-slate-400 text-xs font-semibold uppercase tracking-wider">Rate</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-800/50">
                    {t.assignment.tiers.map((tier, tierIdx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className={`px-3 py-2.5 text-sm ${isActive ? 'text-white' : 'text-slate-400'}`}>
                            {rangeLabel}
                          </td>
                          <td className={`px-3 py-2.5 text-right text-sm font-bold tabular-nums ${isActive ? 'text-amber-400' : 'text-slate-400'}`}>
    ...
    <div className="p-4 md:p-8 max-w-5xl">
      ...
      <div className="mb-6 space-y-2">
        <div className="flex items-center gap-2.5">
          ...
          <div className="h-8 w-48 bg-slate-800 rounded animate-skeleton" style={{ animationDelay: '75ms' }} />
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="card-surface rounded-2xl p-5 space-y-3">
            ...
            <div className="h-8 w-24 bg-slate-800 rounded animate-skeleton" style={{ animationDelay: `${i * 75 + 40}ms` }} />
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-1 bg-slate-900 border border-slate-800 rounded-xl p-1 w-fit mb-6">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="h-9 w-24 bg-slate-800 rounded-lg animate-skeleton" style={{ animationDelay: `${i * 50}ms` }} />
        ))}
      </div>
        ...
        {[...Array(4)].map((_, i) => {
          ...
          return (
            <div key={i} className="card-surface rounded-2xl p-5 flex items-center gap-4">
              {/* Avatar */}
              <div className="w-10 h-10 rounded-full bg-slate-800 flex-shrink-0 animate-skeleton" style={{ animationDelay: `${delay}ms` }} />
              {/* Name + detail */}
              <div className="flex-1 space-y-2">
                <div className="h-4 w-32 bg-slate-800 rounded animate-skeleton" style={{ animationDelay: `${delay}ms` }} />
                <div className="h-3 w-48 bg-slate-800/70 rounded animate-skeleton" style={{ animationDelay: `${delay + 40}ms` }} />
              </div>
              {/* Stats */}
              <div className="flex gap-6">
                {[...Array(3)].map((_, si) => (
                  <div key={si} className="text-center space-y-1">
                    <div className="h-4 w-10 bg-slate-800 rounded animate-skeleton mx-auto" style={{ animationDelay: `${delay + si * 30}ms` }} />
                    <div className="h-3 w-14 bg-slate-800/70 rounded animate-skeleton mx-auto" style={{ animationDelay: `${delay + si * 30}ms` }} />
                  </div>
                ))}
              </div>
            </div>
