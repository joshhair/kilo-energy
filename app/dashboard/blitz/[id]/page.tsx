    ...
    for (const proj of blitz?.projects ?? []) {
      ...
      if (closerId && setterId && closerId === setterId) {
        ...
        if (closerId && participantIds.has(closerId)) {
          ...
          const closerPayout = (setterId && participantIds.has(setterId) ? 0 : m1) + m2 + m3;
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            {isAdmin ? (
              ...
              <div className="card-surface rounded-2xl p-4 animate-slide-in-scale stagger-3">
                ...
                <p className="text-2xl font-bold text-[var(--accent-green)]">{formatCurrency(visibleProjects.reduce((s: number, p: any) => { return s + (p.closer?.id === effectiveRepId ? (p.setter?.id === effectiveRepId ? (p.m1Amount ?? 0) + (p.m2Amount ?? 0) + (p.m3Amount ?? 0) + (p.setterM1Amount ?? 0) + (p.setterM2Amount ?? 0) + (p.setterM3Amount ?? 0) : (p.setter?.id && approvedIds.has(p.setter.id) ? 0 : (p.m1Amount ?? 0)) + (p.m2Amount ?? 0) + (p.m3Amount ?? 0)) : (p.setter?.id === effectiveRepId ? (p.m1Amount ?? 0) + (p.setterM1Amount ?? 0) + (p.setterM2Amount ?? 0) + (p.setterM3Amount ?? 0) : 0)); }, 0))}</p>
              </div>
            )}
          </div>
              ...
              <table className="w-full text-sm">
                ...
                <tfoot>
                  <tr className="border-t border-[var(--border)] bg-[var(--surface-card)]/30">
                    <td colSpan={!isAdmin && !isOwner ? 4 : 3} className="px-4 py-3 text-sm font-semibold text-[var(--text-secondary)]">{visibleProjects.length} deal{visibleProjects.length !== 1 ? 's' : ''}</td>
                    ...
                    {isAdmin && <td className={'px-4 py-3 text-right text-sm font-bold text-white tabular-nums' + (dealsSort.col === 'payout' ? ' bg-[var(--surface-card)]/20' : '')}>{formatCurrency(visibleProjects.reduce((s: number, p: any) => s + (p.m1Amount ?? 0) + (p.m2Amount ?? 0) + (p.m3Amount ?? 0) + (p.setterM1Amount ?? 0) + (p.setterM2Amount ?? 0) + (p.setterM3Amount ?? 0), 0))}</td>}
                  </tr>
                </tfoot>
              </table>
            </div>
