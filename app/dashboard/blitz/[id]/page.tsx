    ...
    for (const proj of blitz?.projects ?? []) {
      ...
      if (closerId && setterId && closerId === setterId) {
        ...
        if (setterId && setterId !== closerId && participantIds.has(setterId)) {
          ...
          bump(setterId, kW, (sM1 || m1) + sM2 + sM3);
        }
      }
    }
                      ...
                      {canManage && (
                        <td className="px-4 py-3 text-right">
                          {p.joinStatus === 'pending' ? (
                            ...
                            p.user.id !== blitz.owner.id && <button onClick={() => setConfirmAction({ title: `Remove ${p.user.firstName} ${p.user.lastName}?`, message: 'This will permanently remove them from the blitz and unlink all their attributed deals. This cannot be undone — re-adding them later will not restore those deal links.', onConfirm: () => { handleRemoveParticipant(p.user.id); setConfirmAction(null); } })} className="text-[#525c72] hover:text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                          )}
                        </td>
                      )}
                    </tr>
