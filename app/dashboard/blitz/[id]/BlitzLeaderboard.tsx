        ...
        {entries.slice(0, 5).map((rep, idx) => {
          ...
          return (
            <div key={rep.userId} className={`flex items-center gap-3 px-3 py-2 rounded-lg border transition-all duration-150 hover:scale-[1.004] ${
              ...
              <Link href={`/dashboard/users/${rep.userId}`} className={`flex-1 text-sm font-medium truncate hover:text-[var(--accent-cyan)] transition-colors ${
              ...
              {rep.payout > 0 && (
                <span className="hidden xl:inline text-xs font-semibold tabular-nums text-[var(--accent-green)] whitespace-nowrap">{formatCurrency(rep.payout)}</span>
              )}
            </div>
          );
        })}
