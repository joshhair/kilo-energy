...
function StageBadge({ stage }: { stage: string }) {
  const color = stage === 'M1' ? 'text-[var(--accent-emerald-solid)] bg-[var(--accent-emerald-solid)]/10 border-[var(--accent-emerald-solid)]/20'
    ...
    <div className="px-3 pt-2 pb-4 md:p-8 max-w-4xl animate-fade-in-up">
      ...
      <ReimbursementModal
        ...
        onSubmit={async (data) => {
          ...
          // Destructure to strip receiptFile from the optimistic Reimbursement;
          // that field is client-only state for the upload chain.
          ...
          try {
            ...
            // Swap the optimistic row for the server-assigned id so the
            // subsequent upload targets the real record.
            setReimbursements((prev) => prev.map((r) => r.id === tempId ? created : r));

            // Chain the receipt upload if the user attached a file.
            if (receiptFile) {
            ...
            console.error(err);
            toast('Failed to submit reimbursement', 'error');
        ...
        <div className="px-4 py-4 md:px-8 md:py-8">
          <div className="flex items-center gap-3 mb-5">
            <div className="p-2 rounded-xl" style={{ backgroundColor: 'rgba(59,130,246,0.12)' }}>
              <PayIcon className="w-5 h-5 text-[var(--accent-emerald-solid)]" />
            </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              ...
              <p className="font-black tabular-nums tracking-tight leading-none break-words"
                 style={{ fontFamily: "'DM Serif Display', serif", color: 'var(--accent-emerald-solid)', textShadow: '0 0 32px rgba(16,185,129,0.30)', fontSize: 'clamp(1.5rem, 8vw, 3rem)' }}>
                {fmt$(nextPayoutTotal)}
              </p>
              <p className="text-[var(--text-muted)] text-xs mt-2">{formatFridayLong(nextFridayStr)}</p>
            </div>
            <span className="inline-flex items-center gap-1.5 bg-[var(--accent-emerald-solid)]/10 border border-[var(--accent-emerald-solid)]/20 text-[var(--accent-emerald-solid)] text-[10px] font-semibold px-2.5 py-1 rounded-full whitespace-nowrap self-start sm:self-end">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        ...
        <div className="card-surface card-surface-stat rounded-2xl p-3 md:p-5 animate-slide-in-scale stagger-1 border-l-2 border-l-emerald-500/40"
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            <DollarSign className="w-4 h-4 text-[var(--accent-emerald-solid)]/50" />
          </div>
          <p className="font-black tabular-nums text-[var(--accent-emerald-solid)] stat-value break-words"
        ...
        <div className="card-surface card-surface-stat rounded-2xl p-3 md:p-5 animate-slide-in-scale stagger-3 col-span-2 sm:col-span-1 border-l-2 border-l-blue-500/40"
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            <TrendingUp className="w-4 h-4 text-[var(--accent-emerald-solid)]/50" />
          </div>
          <p className="font-black tabular-nums text-[var(--accent-emerald-solid)] stat-value break-words"
        ...
        {(projectedM1 > 0 || projectedM2 > 0 || projectedM3 > 0) && (
          <div className="card-surface rounded-2xl p-5 animate-slide-in-scale stagger-6">
            <div className="flex items-center gap-2 mb-4">
              <Calendar className="w-4 h-4 text-[var(--accent-emerald-solid)]" />
                ...
                <div className="card-surface rounded-xl p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg bg-[var(--accent-emerald-solid)]/15 flex items-center justify-center">
                      <span className="text-[var(--accent-emerald-solid)] text-xs font-bold">M1</span>
                    </div>
                    <div>
                  ...
                  <p className="text-[var(--accent-emerald-solid)] font-bold tabular-nums break-words text-right shrink-0 ml-3" style={{ textShadow: '0 0 12px rgba(59,130,246,0.3)' }}>
                    {fmt$(projectedM1)}
                  </p>
                </div>
              )}
      ...
      <div className="flex flex-col md:flex-row flex-wrap items-stretch md:items-center gap-2 md:gap-3 mb-4">
        ...
        {(payFilterFrom || payFilterTo) && (
          <button
            ...
            className="text-xs text-[var(--accent-emerald-solid)] hover:text-emerald-300 font-medium transition-colors whitespace-nowrap"
          >
            Clear
          </button>
        )}
            ...
            {myProjects.length > 0 ? (
              <>
                <p className="text-[var(--text-secondary)] text-sm font-medium">
                  You have <span className="text-[var(--accent-emerald-solid)] font-semibold">{myProjects.length}</span> active deal{myProjects.length !== 1 ? 's' : ''} worth ~<span className="text-[var(--accent-emerald-solid)] font-semibold">${(projectedM1 + projectedM2).toLocaleString()}</span> in projected commissions.
                </p>
                  ...
                  {(() => {
                    ...
                    return preAccCount > 0 ? (
                      <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[var(--accent-emerald-solid)]/10 border border-[var(--accent-emerald-solid)]/20">
                        <Clock className="w-3.5 h-3.5 text-[var(--accent-emerald-solid)]" />
                        <span className="text-[var(--accent-emerald-solid)] text-xs font-medium">{preAccCount} awaiting M1</span>
                      </div>
                    ) : null;
                  })()}
                  {(() => {
              ...
              >
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${
                  ...
                  {period.isUpcoming ? (
                    ...
                    <DollarSign className={`w-4 h-4 ${period.isPast ? 'text-[var(--text-muted)]' : 'text-[var(--accent-emerald-solid)]'}`} />
                  )}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {period.isUpcoming && (
                      <span className="text-[10px] font-semibold text-[var(--accent-emerald-solid)] bg-[var(--accent-emerald-solid)]/10 border border-[var(--accent-emerald-solid)]/20 px-1.5 py-0.5 rounded-full">
                        Next
                      </span>
                    )}
                  </div>
                ...
                <p className={`text-lg font-bold tabular-nums shrink-0 break-words ${period.isUpcoming ? 'text-[var(--accent-emerald-solid)]' : period.isPast ? 'text-[var(--text-secondary)]' : 'text-white'}`}>
                  {fmt$(period.total)}
                </p>
                          ...
                          {period.entries.map((entry) => (
                            <tr key={entry.id} className="border-b border-[var(--border-subtle)]/30 hover:bg-[var(--surface-card)]/20 transition-colors min-h-[44px]">
                              <td className="px-5 py-3">
                                {entry.projectId ? (
                                  <Link href={`/dashboard/projects/${entry.projectId}`} className="text-white hover:text-[var(--accent-emerald-solid)] transition-colors font-medium text-xs">
                                    {entry.customerName || '—'}
                                  </Link>
                                ) : (
                              ...
                              <td className="px-5 py-3 text-right">
                                <span className={`font-semibold tabular-nums ${entry.amount < 0 ? 'text-red-400' : (entry.status === 'Paid' && entry.date <= todayStr ? 'text-[var(--accent-emerald-solid)]' : 'text-white')}`}>
                                  {fmt$(entry.amount)}
                                </span>
                              </td>
